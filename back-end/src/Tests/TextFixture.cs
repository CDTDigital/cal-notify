...
namespace Tests
{
    public class StartupFixture<TStartup> : BaseFixture, IDisposable where TStartup : class
    {
        ...
        public  DbContextOptions<BusinessDbContext> ContextOptions;
        ...
        protected virtual void InitializeServices(IServiceCollection services)
        {
            ...
            mockedSmsSender.Setup(x =>
                x.SendValidationToSms(It.IsAny<TempUser>())).ReturnsAsync(FakeToken);
        ...
        public async Task<ResponseShell<T>> Post<T>(string route, FormUrlEncodedContent queryParams)
        {
            using (var client = _server.CreateClient())
            {
                string query = "";
                ...
                var responseValidate =
                    await (await client.PostAsync(route + query, null)).Content
                        .ReadAsStringAsync();
                client.Dispose();
                return JsonConvert.DeserializeObject<ResponseShell<T>>(responseValidate);
            }
        ...
        public async Task<HttpResponseMessage> Get(string route, FormUrlEncodedContent queryParams)
        {
            using (var client = _server.CreateClient())
            {
                string query = "";
                ...
                return   await client.GetAsync(route + query);
